...
title: API Referansı
sidebar_position: 10
custom_edit_url: https://github.com/gokayybaz/bazntms/edit/main/docs/API.md
---

> Kaynak: [`docs/API.md`](https://github.com/gokayybaz/bazntms/blob/main/docs/API.md) — bu sayfa her build'de otomatik senkronize edilir.
# API Referansı
...
| Uç | Açıklama |
|----|----------|
| `GET /api/v1/agents` | `{id, name, site, online, last_seen, version, remote_ip, attr_method, attr_iface, attr_note, rates[{name, rx_bps, tx_bps, pps, rx_bytes, tx_bytes, rx_packets, tx_packets}], conns}` — `attr_method`: süreç-atıf arka ucu (`ebpf`\|`pcap`\|`etw`\|`off`); `attr_iface`: pcap yakalama arayüzü (yalnız `pcap`); `attr_note`: motor kapalı/başlatılamadıysa neden |
