...
| Uç | Açıklama |
|----|----------|
| `GET /api/v1/agents` | `{id, name, site, online, last_seen, version, remote_ip, attr_method, rates[{name, rx_bps, tx_bps, pps, rx_bytes, tx_bytes, rx_packets, tx_packets}], conns}` — `attr_method`: süreç-atıf arka ucu (`ebpf`\|`pcap`\|`etw`\|`off`) |
