...
func TestSevRank(t *testing.T) {
	...
	tr := NewTriager(nil, nil, "crit", 10, nil, 0, nil)
...
func TestNewTriagerDefaults(t *testing.T) {
	tr := NewTriager(nil, nil, "", 0, nil, 0, nil)
...
func TestJevWorthTriage(t *testing.T) {
	...
	tr = NewTriager(nil, nil, "crit", 10, fakeJev{noul: 0.2}, 0.55, nil)
	if tr.jevWorthTriage(in) {
		t.Fatal("noul eşiğin altında — atlanmalıydı")
	}

	// yüksek güven → devam etmeli
	tr = NewTriager(nil, nil, "crit", 10, fakeJev{noul: 0.9}, 0.55, nil)
	if !tr.jevWorthTriage(in) {
		t.Fatal("noul eşiğin üstünde — devam etmeliydi")
	}

	// hata → fail-open (devam etmeli)
	tr = NewTriager(nil, nil, "crit", 10, fakeJev{err: fmt.Errorf("bağlantı hatası")}, 0.55, nil)
	if !tr.jevWorthTriage(in) {
		t.Fatal("jev hatasında fail-open olmalı (devam etmeli)")
	}
}

func TestJevWorthTriagePersistsDecision(t *testing.T) {
	in := store.Incident{ID: 7, Severity: "crit"}
	var gotID int64
	var gotNoul float64
	var gotWorth bool
	tr := NewTriager(nil, nil, "crit", 10, fakeJev{noul: 0.9}, 0.55, func(id int64, noul float64, worth bool) error {
		gotID, gotNoul, gotWorth = id, noul, worth
		return nil
	})
	if !tr.jevWorthTriage(in) {
		t.Fatal("noul eşiğin üstünde — devam etmeliydi")
	}
	if gotID != 7 || gotNoul != 0.9 || !gotWorth {
		t.Fatalf("setJevDecision çağrısı = id=%d noul=%v worth=%v", gotID, gotNoul, gotWorth)
	}

	// fail-open yolunda (hata) setJevDecision ÇAĞRILMAMALI — jev_noul -1
	// sentinel'inde kalmalı ("değerlendirilmedi" ile "değmez" karışmasın).
	called := false
	tr = NewTriager(nil, nil, "crit", 10, fakeJev{err: fmt.Errorf("bağlantı hatası")}, 0.55,
		func(id int64, noul float64, worth bool) error { called = true; return nil })
	tr.jevWorthTriage(in)
	if called {
		t.Fatal("fail-open yolunda setJevDecision çağrılmamalı")
	}
}
